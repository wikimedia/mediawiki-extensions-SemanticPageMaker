		...
		for ( $level = $num_levels; $level > 0; $level-- ) {
			...
			foreach ( $checkcategories as $category ) {
				if ( $substring != null ) {
					...
					$conditions = 'cl_to = ' . $db->addQuotes( $category ) . " AND (LOSPMR(CONVERT(`page_title` USING utf8)) LIKE '" . $substring . "%' OR LOSPMR(CONVERT(`page_title` USING utf8)) LIKE '%\_" . $substring . "%' OR page_namespace = " . NS_CATEGORY . ")";
				} else {
					...
					while ( $res && $row = $db->fetchRow( $res ) ) {
						if ( array_key_exists( 'page_title', $row ) ) {
							...
							if ( $page_namespace == NS_CATEGORY ) {
								...
								$cur_value = ( $page_namespace == NS_MAIN ? '' : $ns[$page_namespace] . ':' ) . str_replace( "_", " ", $row['page_title'] );
								if ( ! in_array( $cur_value, $pages ) ) {
								// autocomplete patch, == to ===
									if ( $substring === null )
										$pages[] = $cur_value;
									else
...
*/
		...
		foreach ( $namespaces as $ns_code => $ns_name ) {
			if ( $ns_name == $namespace_name ) {
				...
				if ( $substring != null ) {
					...
					$substring = str_replace( '_', '\_', $substring );
					$substring = str_replace( "'", "\'", $substring );
					$conditions .= " AND (LOSPMR(CONVERT(`page_title` USING utf8)) LIKE '$substring%' OR LOSPMR(CONVERT(`page_title` USING utf8)) LIKE '%\_$substring%')";
				}
	...
	// autocomplete patch
	static function getAllPagesForQuery( $querystring, $substring = null ) {
		global $sfgMaxAutocompleteValues, $smwgQDefaultNamespaces, $smwgQFeatures, $wgParser, $wgTitle;
		$options = new ParserOptions;
		$querystring = $wgParser->preprocess( $querystring, $wgTitle, $options );
		...
		$pages = array();
		while ( $res = $query_result->getNext() ) {
			$pages[] = $res[0]->getNextObject()->getWikiValue();
		}
		return $pages;
	}
}
