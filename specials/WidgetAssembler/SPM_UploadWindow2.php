<?php
/**
 * Code from SemanticForms v2.0.0_1 [B25]
 * specials/SF_UploadWindow2.php
 *
 * small change for clean wiki output
 */
class SPMUploadWindow2 extends UnlistedSpecialPage {
	/**
	...
	protected function loadRequest( $request ) {
		...
		// FIXME: session will not update inside iframes
	...
	protected function showViewDeletedLinks() {
		...
		if ( $title instanceof Title && ( $count = $title->isDeleted() ) > 0 && $wgUser->isAllowed( 'deletedhistory' ) ) {
			...
			->rawParams(
				Linker::linkKnown(
					...
					wfMessage( 'restorelink', $count )->parse()
				)
			)->parse();
		...
		if ( $title instanceof Title && $title->isDeletedQuick() && !$title->exists() ) {
			$this->showDeletionLog( $wgOut, SPMWidgetUtils::getTitlePrefixedText( $title ) );
		}
	}
	...
	protected function uploadWarning( $warnings ) {
		...
		foreach ( $warnings as $warning => $args ) {
				if ( $warning == 'exists' ) {
					...
					$msg = "\t<li>" . wfMessage( 'file-deleted-duplicate',
								array( SPMWidgetUtils::getTitlePrefixedText( Title::makeTitle( NS_FILE, $args ) ) ) )
							->parse()
						. "</li>\n";
				} else {
					if ( is_bool( $args ) )
		...
		$warningHtml .= "</ul>\n";
		$warningHtml .= wfMessage( 'uploadwarning-text' )->parse();
		...
		switch( $details['status'] ) {
			...
			case UploadBase::FILETYPE_BADTYPE:
				...
				$this->uploadError(
					...
					->numParams( count( $wgFileExtensions ) )
					->parse()
				);
				break;
			...
			case UploadBase::HOOK_ABORTED:
				...
				break;
			default:
				throw new MSPMxception( __METHOD__ . ": Unknown value `{$details['status']}`" );
		}
	}
	...
	public static function getExistsWarning( $exists ) {
		...
		if ( $exists['warning'] == 'exists' ) {
			...
			$warning[] = '<li>' . wfMessage( 'fileexists-extension', $filename,
				SPMWidgetUtils::getTitlePrefixedText( $exists['normalizedFile']->getTitle() ) )->parse() . '</li>';
		} elseif ( $exists['warning'] == 'thumb' ) {
			// Swapped argument order compared with other messages for backwards compatibility
			$warning[] = '<li>' . wfMessage( 'fileexists-thumbnail-yes',
				SPMWidgetUtils::getTitlePrefixedText( $exists['thumbFile']->getTitle() ), $filename )->parse() . '</li>';
			...
			$warning[] = '<li>' . wfMessage( 'filewasdeleted', $llink )->escape() . '</li>';
		}
	...
	public static function getDupeWarning( $dupes ) {
		if ( $dupes ) {
			...
			foreach ( $dupes as $file ) {
				$title = $file->getTitle();
				$msg .= SPMWidgetUtils::getTitlePrefixedText( $title ) .
					"|" . $title->getText() . "\n";
			}
			$msg .= "</gallery>";
			return "<li>" .
				wfMessage( "file-exists-duplicate", count( $dupes ) )->parse() .
				$wgOut->parse( $msg ) .
				"</li>\n";
		} else {
			return '';
		...
		if ( $wgCheckFileExtensions ) {
			if ( $wgStrictFileExtensions ) {
				...
				$extensionsList =
					...
					wfMessage( 'upload-permitted', $wgLang->commaList( $wgFileExtensions ) )->escaped() .
					"</div>\n";
			} else {
				# We have to list both preferred and prohibited
				$extensionsList =
					'<div id="mw-upload-preferred">' .
					wfMessage( 'upload-preferred', $wgLang->commaList( $wgFileExtensions ) )->escaped() .
					"</div>\n" .
					'<div id="mw-upload-prohibited">' .
					wfMessage( 'upload-prohibited', $wgLang->commaList( $wgFileBlacklist ) )->escaped() .
					"</div>\n";
			}
		} else {
